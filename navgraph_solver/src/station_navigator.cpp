...
void makelineMarker(std::string marker1, std::string marker2);
...
void missionPlanner(std::string mission){
	...
	for(int i =0; ; i = (i+1)%stations.size()){
		...
		if(server->get(stations[i], int_marker_point)){
			ROS_INFO("Goal: %f,%f\n", int_marker_point.pose.position.x, int_marker_point.pose.position.y);
      	...
      	if(feedback->menu_entry_id == 4){
	        if(button_clicked){
		      	if(clicked_marker == feedback->marker_name){
		      		...
		      		makelineMarker(clicked_marker, feedback->marker_name);
    ...
    case visualization_msgs::InteractiveMarkerFeedback::POSE_UPDATE:
	    {
			...
			for(int i = 0; i<server->size(); i++){
				...
				if(server->get(s.str(), marker)){
					if(server2->get(s1.str(), marker)){
						makelineMarker(s.str(), feedback->marker_name);
						server2->applyChanges();
					}
					if(server2->get(s2.str(), marker)){
						makelineMarker(feedback->marker_name, s.str());
						server2->applyChanges();
					}
				}
				else{
...
void makelineMarker(std::string marker1, std::string marker2){
	...
	InteractiveMarker int_marker_point;
  	...
  	s << "Connection between " << marker1 << " and " << marker2;
  	ss1 << marker1 << "," << marker2;
  	ss2 << marker2 << "," << marker1;
...
void goalstatusCallback(const actionlib_msgs::GoalStatusArrayConstPtr& msg){
	if(!msg->status_list.empty()){
		char buffer[200];
		std::size_t length = msg->status_list[msg->status_list.size() - 1].text.copy(buffer,13,0);
		buffer[length]='\0';
...
int main(int argc, char **argv){
	...
	// missionPlanner("1 1 1 180/3 1 2 90/5 0 7 -90.0/6 0 1 0/0 1 1 0");
	// missionPlanner("1 1 1/3 1 2/5 1 7/6 1 1/0 1 1");
	ros::waitForShutdown();
