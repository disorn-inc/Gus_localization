...
def message_to_csv(stream, msg, flatten=False):
    ...
    try:
        ...
        msg_str = str(msg)
        if msg_str.find(",") is not -1: #Hay comas en el mensaje. Si es -1 es porque no hay comas.
            if flatten:
                msg_str = msg_str.strip("(") #Eliminar character "("
                msg_str = msg_str.strip(")") #Eliminar character ")"
                msg_str = msg_str.strip(" ") #Eliminar character " "
            else:
                msg_str = "\"" + msg_str + "\""
...
def main(options):
    ...
    if len(options.topic_names)==0: #si no se selecciona ningun topic
