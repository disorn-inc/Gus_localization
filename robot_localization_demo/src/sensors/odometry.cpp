...
namespace robot_localization_demo {
  ...
  void TurtleOdometry::spawnAndConfigureVisualizationTurtle(const turtlesim::Pose & initial_pose) {
    if(isVisualizationRequested() && !isVisualizationTurtleAvailable()) {
      ...
      spawn_visualization_turtle.request.theta = yaw;      auto client = node_handle_.serviceClient<decltype(spawn_visualization_turtle)>("spawn");
      */
      ...
      // Log message.
      ROS_INFO("Relative position measurement (odometry) visualized by '%s' with a green pen.", visualization_turtle_name_.c_str());
    }
  }
