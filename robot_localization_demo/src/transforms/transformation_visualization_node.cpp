...
int main(int argc, char** argv) {
  ...
  // Log message.
  ROS_INFO("Absolute position EKF estimate visualized by '%s' using a red pen.", visualization_turtle_name.c_str());
  ...
  while(node_handle.ok()) {
    ...
    tf::Quaternion quaternion(pose_map.pose.orientation.x, pose_map.pose.orientation.y, pose_map.pose.orientation.z, pose_map.pose.orientation.w);
    //tf2::Quaternion quaternion(pose_map.pose.orientation.y, -pose_map.pose.orientation.z, -pose_map.pose.orientation.w, pose_map.pose.orientation.x);
    ...
    visualize_current_pose.request.theta = yaw;
